...
Sub Step00_Setup()
    ...
    ' ---- SAVE AS A001901E.sldprt (next to the macros folder) ----
    Dim macroPath As String, savePath As String
    Dim saveErrs As Long, saveWarns As Long
    macroPath = swApp.GetCurrentMacroPathName
    savePath = Left$(macroPath, InStrRev(macroPath, "\")) & "..\A001901E.sldprt"
...
Sub Step01_F001()
    ...
    ' ---- SKETCH: rectangle 80 x 2.5, lower-left corner at (0, 0) ----
    ...
    swModel.SketchManager.CreateCornerRectangle 0 * UNIT_FACTOR, 0 * UNIT_FACTOR, 0#, _
        (0 + 80) * UNIT_FACTOR, (0 + 2.5) * UNIT_FACTOR, 0#
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F001_Base_T_section_I_section_extrusion_Front"
    If Not VerifySolidBody("01_F001") Then
        ...
        End
    End If
    LogResult "PASS", "01_F001", "Created feature F001_Base_T_section_I_section_extrusion_Front"
End Sub
...
Sub Step02_F002()
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: 9 hole(s) dia 1.125 (counterbore) ----
    swModel.SketchManager.CreateCircleByRadius 1.5 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.125 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 10 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.125 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 20 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.125 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 30 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.125 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 40 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.125 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 50 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.125 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 60 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.125 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 70 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.125 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 77 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.125 / 2#) * UNIT_FACTOR
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F002_9x_counterbore_holes_1_125_dia_thru_with"
    If Not VerifySolidBody("02_F002") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "02_F002", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "02_F002", "Created feature F002_9x_counterbore_holes_1_125_dia_thru_with"

    ' ---- COUNTERBORE: concentric blind cut dia 1.625 x 1 deep ----
    ' ---- PLANE SELECTION (Front Plane; name auto-detected) ----
    If Not SelectRefPlane("Front Plane", 1) Then
        MsgBox "Could not select Front Plane (no reference plane found).", vbCritical
        LogResult "FAIL", "02_F002_cbore", "Could not select Front Plane (no reference plane found)."
        End
    End If

    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    swModel.SketchManager.CreateCircleByRadius 1.5 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.625 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 10 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.625 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 20 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.625 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 30 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.625 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 40 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.625 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 50 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.625 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 60 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.625 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 70 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.625 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 77 * UNIT_FACTOR, 4.375 * UNIT_FACTOR, 0#, (1.625 / 2#) * UNIT_FACTOR
    ...
    If swModel.SketchManager.ActiveSketch Is Nothing Then
        MsgBox "No active sketch to build the feature from.", vbCritical
        LogResult "FAIL", "02_F002_cbore", "No active sketch to build the feature from."
        End
    End If
    ...
    If swFeatCb Is Nothing Then
        ...
        LogResult "FAIL", "02_F002", "Counterbore cut failed."
        End
    End If
    swFeatCb.Name = "F002_cbore"
    LogResult "PASS", "02_F002", "Counterbore created"
End Sub
...
Sub StepZZ_FinalVerify()
    ...
    ' Expected from the drawing: length=80; length=83; height=7.25
    ...
    MsgBox "Bounding box (drawing units): " & _
        ...
        "Drawing envelope: length=80; length=83; height=7.25" & vbCrLf & _
        "Expected feature count: 2", vbInformation
