...
Sub Step00_Setup()
    ...
    ' ---- SAVE AS A001821M.sldprt (next to the macros folder) ----
    Dim macroPath As String, savePath As String
    Dim saveErrs As Long, saveWarns As Long
    macroPath = swApp.GetCurrentMacroPathName
    savePath = Left$(macroPath, InStrRev(macroPath, "\")) & "..\A001821M.sldprt"
...
Sub Step01_F001()
    ...
    ' ---- SKETCH: rectangle 4.5 x 4.5, lower-left corner at (0, 0) ----
    ...
    swModel.SketchManager.CreateCornerRectangle 0 * UNIT_FACTOR, 0 * UNIT_FACTOR, 0#, _
        (0 + 4.5) * UNIT_FACTOR, (0 + 4.5) * UNIT_FACTOR, 0#
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F001_Base_plate_stepped_L_shaped_profile_extr"
    If Not VerifySolidBody("01_F001") Then
        ...
        End
    End If
    LogResult "PASS", "01_F001", "Created feature F001_Base_plate_stepped_L_shaped_profile_extr"
End Sub
...
Sub Step02_F002()
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: rectangle 3.875 x 3.875, lower-left corner at (0, 0) ----
    ' (Corner at the origin keeps sketch coordinates equal to the drawing's
    '  edge-referenced dimensions, so hole positions land where dimensioned.)
    swModel.SketchManager.CreateCornerRectangle 0 * UNIT_FACTOR, 0 * UNIT_FACTOR, 0#, _
        (0 + 3.875) * UNIT_FACTOR, (0 + 3.875) * UNIT_FACTOR, 0#
    ' NOTE: POSITION ASSUMED (drawing frame: rect corner at origin / circle at plate center) - verify against the drawing.
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F002_Step_cut_removing_the_upper_right_corner"
    If Not VerifySolidBody("02_F002") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "02_F002", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "02_F002", "Created feature F002_Step_cut_removing_the_upper_right_corner"
End Sub
...
Sub Step03_F003()
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: 1 hole(s) dia 0.236 (tapped) ----
    swModel.SketchManager.CreateCircleByRadius 0 * UNIT_FACTOR, 2.25 * UNIT_FACTOR, 0#, (0.236 / 2#) * UNIT_FACTOR
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F003_M6x1_tapped_through_holes_7_total_arrang"
    If Not VerifySolidBody("03_F003") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "03_F003", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "03_F003", "Created feature F003_M6x1_tapped_through_holes_7_total_arrang"

    ' TODO: VERIFY API CALL - cosmetic thread for "M6x1"
    ...
    ' spec "M6x1". (InsertCosmeticThread3 exists but its argument
    ' shape was not verified against a documented example, so it is not scripted.)
    LogResult "WARN", "03_F003", "Apply cosmetic thread M6x1 manually - see macro comments"
End Sub
...
Sub Step04_F004()
    ...
    MsgBox "POSITION ASSUMED for hole H002: centered/laid-out from the envelope; verify hole locations in SolidWorks.", vbExclamation, "Verify before continuing (H002)"
    ' ---- PLANE SELECTION (Front Plane; name auto-detected) ----
    If Not SelectRefPlane("Front Plane", 1) Then
        MsgBox "Could not select Front Plane (no reference plane found).", vbCritical
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: 1 hole(s) dia 0.422 (thru) ----
    swModel.SketchManager.CreateCircleByRadius 0 * UNIT_FACTOR, 2.25 * UNIT_FACTOR, 0#, (0.422 / 2#) * UNIT_FACTOR
    ' NOTE: HOLE POSITIONS ASSUMED (centered on the plate envelope) - verify against the drawing.
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F004_Dia_0_422_clearance_through_holes_6_tota"
    If Not VerifySolidBody("04_F004") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "04_F004", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "04_F004", "Created feature F004_Dia_0_422_clearance_through_holes_6_tota"
End Sub
...
Sub Step05_F005()
    ...
    ' TODO: VERIFY API CALL — cosmetic thread for F005
    ' Apply via Insert > Annotations > Cosmetic Thread. M6x1 internal thread applied to the 7 tapped holes
    LogResult "WARN", "05_F005", "F005 cosmetic thread - apply manually"
End Sub
...
Sub StepZZ_FinalVerify()
    ...
    ' Expected from the drawing: width=4.5; height=7; height=6.25
    ...
    MsgBox "Bounding box (drawing units): " & _
        ...
        "Drawing envelope: width=4.5; height=7; height=6.25" & vbCrLf & _
