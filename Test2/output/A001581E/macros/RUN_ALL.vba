...
Sub Step00_Setup()
    ...
    ' ---- SAVE AS A001581E.sldprt (next to the macros folder) ----
    Dim macroPath As String, savePath As String
    Dim saveErrs As Long, saveWarns As Long
    macroPath = swApp.GetCurrentMacroPathName
    savePath = Left$(macroPath, InStrRev(macroPath, "\")) & "..\A001581E.sldprt"
...
Sub Step01_F001()
    ...
    ' ---- SKETCH: rectangle 11 x 11, lower-left corner at (0, 0) ----
    ...
    swModel.SketchManager.CreateCornerRectangle 0 * UNIT_FACTOR, 0 * UNIT_FACTOR, 0#, _
        (0 + 11) * UNIT_FACTOR, (0 + 11) * UNIT_FACTOR, 0#
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F001_Base_flat_plate_11_00_wide_x_6_25_tall_x"
    If Not VerifySolidBody("01_F001") Then
        ...
        End
    End If
    LogResult "PASS", "01_F001", "Created feature F001_Base_flat_plate_11_00_wide_x_6_25_tall_x"
End Sub
...
Sub Step02_F002()
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: rectangle 1.62 x 1.62, lower-left corner at (1.56, 4.36) ----
    ' (Corner at the origin keeps sketch coordinates equal to the drawing's
    '  edge-referenced dimensions, so hole positions land where dimensioned.)
    swModel.SketchManager.CreateCornerRectangle 1.56 * UNIT_FACTOR, 4.36 * UNIT_FACTOR, 0#, _
        (1.56 + 1.62) * UNIT_FACTOR, (4.36 + 1.62) * UNIT_FACTOR, 0#
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F002_Rectangular_notch_slot_cut_from_top_edge"
    If Not VerifySolidBody("02_F002") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "02_F002", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "02_F002", "Created feature F002_Rectangular_notch_slot_cut_from_top_edge"
End Sub
...
Sub Step03_F003()
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: 6 hole(s) dia 0.218 (thru) ----
    swModel.SketchManager.CreateCircleByRadius 0.25 * UNIT_FACTOR, 0.375 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 0.25 * UNIT_FACTOR, 3.125 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 0.25 * UNIT_FACTOR, 5.875 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 10.75 * UNIT_FACTOR, 0.375 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 10.75 * UNIT_FACTOR, 3.125 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 10.75 * UNIT_FACTOR, 5.875 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F003_218_diameter_thru_holes_6_total_3_on_lef"
    If Not VerifySolidBody("03_F003") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "03_F003", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "03_F003", "Created feature F003_218_diameter_thru_holes_6_total_3_on_lef"
End Sub
...
Sub Step04_FilletsChamfers()
    ...
    ' ---- F004: FILLET R0.25 (.25 R TYP fillets on notch corners) ----
    ...
    Else
        ...
        Else
            swFeatF004.Name = "F004_25_R_TYP_fillets_on_notch_corners"
...
Sub StepZZ_FinalVerify()
    ...
    ' Expected from the drawing: width=11; height=6.25
    ...
    MsgBox "Bounding box (drawing units): " & _
        ...
        "Drawing envelope: width=11; height=6.25" & vbCrLf & _
