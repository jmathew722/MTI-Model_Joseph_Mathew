...
Sub Step00_Setup()
    ...
    ' ---- SAVE AS A001591E.sldprt (next to the macros folder) ----
    Dim macroPath As String, savePath As String
    Dim saveErrs As Long, saveWarns As Long
    macroPath = swApp.GetCurrentMacroPathName
    savePath = Left$(macroPath, InStrRev(macroPath, "\")) & "..\A001591E.sldprt"
...
Sub Step01_F001()
    ...
    ' ---- SKETCH: rectangle 8 x 8, lower-left corner at (0, 0) ----
    ...
    swModel.SketchManager.CreateCornerRectangle 0 * UNIT_FACTOR, 0 * UNIT_FACTOR, 0#, _
        (0 + 8) * UNIT_FACTOR, (0 + 8) * UNIT_FACTOR, 0#
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F001_Base_flat_plate_8_00_x_7_00_in_extruded_"
    If Not VerifySolidBody("01_F001") Then
        ...
        End
    End If
    LogResult "PASS", "01_F001", "Created feature F001_Base_flat_plate_8_00_x_7_00_in_extruded_"
End Sub
...
Sub Step02_F002()
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: 8 hole(s) dia 0.218 (thru) ----
    swModel.SketchManager.CreateCircleByRadius 0.25 * UNIT_FACTOR, 0.25 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 4 * UNIT_FACTOR, 0.25 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 7.75 * UNIT_FACTOR, 0.25 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 0.25 * UNIT_FACTOR, 3.5 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 7.75 * UNIT_FACTOR, 3.5 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 0.25 * UNIT_FACTOR, 6.75 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 4 * UNIT_FACTOR, 6.75 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 7.75 * UNIT_FACTOR, 6.75 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F002_8x_218_dia_thru_holes_on_7_500_x_6_500_b"
    If Not VerifySolidBody("02_F002") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "02_F002", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "02_F002", "Created feature F002_8x_218_dia_thru_holes_on_7_500_x_6_500_b"
End Sub
...
Sub Step03_FilletsChamfers()
    ...
    ' ---- F003: FILLET R0.12 (.12 R. TYP corner radii on all four corners of base plate profile) ----
    If swSelMgr.GetSelectedObjectCount2(-1) = 0 Then
        MsgBox "Select the edge(s) for fillet F003 (R0.12), then run again.", vbExclamation
    ...
    Else
        ...
        Else
            swFeatF003.Name = "F003_12_R_TYP_corner_radii_on_all_four_corner"
            LogResult "PASS", "03_fillets_chamfers", "F003 fillet R0.12 applied"
        End If
        swModel.ClearSelection2 True
    End If
End Sub
...
Sub StepZZ_FinalVerify()
    ...
    ' Expected from the drawing: width=8; height=7
    ...
    MsgBox "Bounding box (drawing units): " & _
        ...
        "Drawing envelope: width=8; height=7" & vbCrLf & _
