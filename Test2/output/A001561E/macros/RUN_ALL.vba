...
Sub Step00_Setup()
    ...
    ' ---- SAVE AS A001561E.sldprt (next to the macros folder) ----
    Dim macroPath As String, savePath As String
    Dim saveErrs As Long, saveWarns As Long
    macroPath = swApp.GetCurrentMacroPathName
    savePath = Left$(macroPath, InStrRev(macroPath, "\")) & "..\A001561E.sldprt"
...
Sub Step01_F001()
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F001_Base_flat_sheet_metal_panel_11_00_in_wid"
    If Not VerifySolidBody("01_F001") Then
        ...
        End
    End If
    LogResult "PASS", "01_F001", "Created feature F001_Base_flat_sheet_metal_panel_11_00_in_wid"
End Sub
...
Sub Step02_F003()
    ...
    If Not SelectRefPlane("Front Plane", 1) Then
        MsgBox "Could not select Front Plane (no reference plane found).", vbCritical
        LogResult "FAIL", "02_F003", "Could not select Front Plane (no reference plane found)."
        End
    End If

    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: circle dia 3.062 at (5.5, 0) drawing units ----
    swModel.SketchManager.CreateCircleByRadius 5.5 * UNIT_FACTOR, 0 * UNIT_FACTOR, 0#, (3.062 / 2#) * UNIT_FACTOR
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F003_Semi_circular_cutout_on_the_bottom_edge_"
    If Not VerifySolidBody("02_F003") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "02_F003", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "02_F003", "Created feature F003_Semi_circular_cutout_on_the_bottom_edge_"
End Sub
...
Sub Step03_F002()
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: 7 hole(s) dia 0.218 (thru) ----
    swModel.SketchManager.CreateCircleByRadius 0.25 * UNIT_FACTOR, 5.875 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 5.5 * UNIT_FACTOR, 5.875 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 10.75 * UNIT_FACTOR, 5.875 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 0.25 * UNIT_FACTOR, 2.875 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    swModel.SketchManager.CreateCircleByRadius 10.75 * UNIT_FACTOR, 2.875 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F002_218_in_diameter_through_holes_7_total_ar"
    If Not VerifySolidBody("03_F002") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "03_F002", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "03_F002", "Created feature F002_218_in_diameter_through_holes_7_total_ar"
End Sub
...
Sub StepZZ_FinalVerify()
    ...
    ' Expected from the drawing: width=11; height=6.5
    ...
    MsgBox "Bounding box (drawing units): " & _
        ...
        "Drawing envelope: width=11; height=6.5" & vbCrLf & _
