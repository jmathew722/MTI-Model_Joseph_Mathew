...
Sub Step00_Setup()
    ...
    ' ---- SAVE AS A001551E.sldprt (next to the macros folder) ----
    Dim macroPath As String, savePath As String
    Dim saveErrs As Long, saveWarns As Long
    macroPath = swApp.GetCurrentMacroPathName
    savePath = Left$(macroPath, InStrRev(macroPath, "\")) & "..\A001551E.sldprt"
...
Sub Step01_F001()
    ...
    ' ---- SKETCH: rectangle 21 x 21, lower-left corner at (0, 0) ----
    ...
    swModel.SketchManager.CreateCornerRectangle 0 * UNIT_FACTOR, 0 * UNIT_FACTOR, 0#, _
        (0 + 21) * UNIT_FACTOR, (0 + 21) * UNIT_FACTOR, 0#
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F001_Base_rectangular_plate_21_00_in_long_x_6"
    If Not VerifySolidBody("01_F001") Then
        ...
        End
    End If
    LogResult "PASS", "01_F001", "Created feature F001_Base_rectangular_plate_21_00_in_long_x_6"
End Sub
...
Sub Step02_F002()
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: rectangle 1 x 0.515, lower-left corner at (0.75, 1) ----
    ' (Corner at the origin keeps sketch coordinates equal to the drawing's
    '  edge-referenced dimensions, so hole positions land where dimensioned.)
    swModel.SketchManager.CreateCornerRectangle 0.75 * UNIT_FACTOR, 1 * UNIT_FACTOR, 0#, _
        (0.75 + 1) * UNIT_FACTOR, (1 + 0.515) * UNIT_FACTOR, 0#
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F002_4x_corner_oval_slot_cutouts_0_515_wide_x"
    If Not VerifySolidBody("02_F002") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "02_F002", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "02_F002", "Created feature F002_4x_corner_oval_slot_cutouts_0_515_wide_x"
End Sub
...
Sub Step03_F003()
    ...
    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: rectangle 0.515 x 0.515, lower-left corner at (7.5, 0.187) ----
    ' (Corner at the origin keeps sketch coordinates equal to the drawing's
    '  edge-referenced dimensions, so hole positions land where dimensioned.)
    swModel.SketchManager.CreateCornerRectangle 7.5 * UNIT_FACTOR, 0.187 * UNIT_FACTOR, 0#, _
        (7.5 + 0.515) * UNIT_FACTOR, (0.187 + 0.515) * UNIT_FACTOR, 0#
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F003_2x_center_vertical_slot_cutouts_0_515_wi"
    If Not VerifySolidBody("03_F003") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "03_F003", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "03_F003", "Created feature F003_2x_center_vertical_slot_cutouts_0_515_wi"
End Sub
...
Sub StepZZ_FinalVerify()
    ...
    ' Expected from the drawing: length=21; height=6
    ...
    MsgBox "Bounding box (drawing units): " & _
        ...
        "Drawing envelope: length=21; height=6" & vbCrLf & _
