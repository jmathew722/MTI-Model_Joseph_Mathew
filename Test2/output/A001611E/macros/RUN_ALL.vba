...
Sub Step00_Setup()
    ...
    ' ---- SAVE AS A001611E.sldprt (next to the macros folder) ----
    Dim macroPath As String, savePath As String
    Dim saveErrs As Long, saveWarns As Long
    macroPath = swApp.GetCurrentMacroPathName
    savePath = Left$(macroPath, InStrRev(macroPath, "\")) & "..\A001611E.sldprt"
...
Sub Step01_F001()
    ...
    ' ---- SKETCH: rectangle 32.25 x 32.25, lower-left corner at (0, 0) ----
    ...
    swModel.SketchManager.CreateCornerRectangle 0 * UNIT_FACTOR, 0 * UNIT_FACTOR, 0#, _
        (0 + 32.25) * UNIT_FACTOR, (0 + 32.25) * UNIT_FACTOR, 0#
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F001_Base_extrusion_of_the_lens_diamond_shape"
    If Not VerifySolidBody("01_F001") Then
        ...
        End
    End If
    LogResult "PASS", "01_F001", "Created feature F001_Base_extrusion_of_the_lens_diamond_shape"
End Sub
...
Sub Step02_F002()
    ...
    If Not SelectRefPlane("Right Plane", 3) Then
        ...
        LogResult "FAIL", "02_F002", "Could not select Right Plane (no reference plane found)."
        End
    End If

    ' ---- OPEN SKETCH ----
    swModel.SketchManager.InsertSketch True
    ' ---- SKETCH: 1 hole(s) dia 0.218 (thru) ----
    swModel.SketchManager.CreateCircleByRadius 16.125 * UNIT_FACTOR, 0 * UNIT_FACTOR, 0#, (0.218 / 2#) * UNIT_FACTOR
    ...
    If swFeat Is Nothing Then
        ...
        End
    End If
    swFeat.Name = "F002_218_DR_THRU_holes_along_a_row_at_0_50_in"
    If Not VerifySolidBody("02_F002") Then
        MsgBox "No solid body after this feature.", vbCritical
        LogResult "FAIL", "02_F002", "No solid body after this feature."
        End
    End If
    LogResult "PASS", "02_F002", "Created feature F002_218_DR_THRU_holes_along_a_row_at_0_50_in"
End Sub
...
Sub Step03_FilletsChamfers()
    ...
    ' ---- F003: FILLET R3 (3.00 R. TYP fillet on the corners of the diamond/lens profile sketch. Applied to all sharp corners of the base profile.) ----
    If swSelMgr.GetSelectedObjectCount2(-1) = 0 Then
        MsgBox "Select the edge(s) for fillet F003 (R3), then run again.", vbExclamation
    ...
    Else
        ...
        Else
            swFeatF003.Name = "F003_3_00_R_TYP_fillet_on_the_corners_of_the_"
            LogResult "PASS", "03_fillets_chamfers", "F003 fillet R3 applied"
        End If
        swModel.ClearSelection2 True
    End If
End Sub
...
Sub StepZZ_FinalVerify()
    ...
    ' Expected from the drawing: length=32.25
    ...
    MsgBox "Bounding box (drawing units): " & _
        ...
        "Drawing envelope: length=32.25" & vbCrLf & _
